...
static int Lassert(softwarestate &state, int nargs) {
...
static int Lerror(softwarestate &state, int nargs) {
	throw exception(nargs >= 1 ? state[1].tostring(state) : "expected 'string' for argument #1 to 'error'");
	return 0;
}

static int print(softwarestate &state, int nargs) {
...
static int rawset(softwarestate &state, int nargs) {
...
static int Lselect(softwarestate &state, int nargs) {
	auto amount = state[1].tonumber(state);

	return std::max(nargs - 1, (int)amount);
}

static int tostring(softwarestate &state, int nargs) {
	state.loadstring(state[1].tostring(state));

	return 1;
}

static int type(softwarestate &state, int nargs) {
